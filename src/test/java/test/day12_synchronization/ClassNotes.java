...
- PAGE OBJECT MODEL DESIGN PATTERN
	...
	- DISADVANTAGES:
		...
		-> But the more web elements we create in the beginning, the less work is left for us in the future. So later on, we will just keep re-using the methods.
...
When we use find element method:
	...
	--> if you already have set implicit wait:
		-> if WebElement is not found
			-> it will trigger the implicitlyWait method. (10, TimeUnit.sec)
			-> it will wait for the given time
			-> in that given seconds if driver can find the web element we are looking for, it will return the web element
