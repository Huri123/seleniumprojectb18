...
public class SmartBearTests {
    ...
    public void p2_smartbear_order_placing() throws InterruptedException {
        ...
        //5. Click on Login button  //Buraya kadar beforeMethod un icinde
        //6. Click on Order
        ...
        //quantityInput.clear(); // just deletes if there is any existing input in the input box.(AMA BURDA BU ISE YARAMADI)
        Thread.sleep(1000);
        ...
        //Entering name using java faker
        inputCustomerName.sendKeys(faker.name().fullName());

        //Entering street using java faker
        inputStreet.sendKeys(faker.address().streetAddress());

        //Entering city using java faker
        inputCity.sendKeys(faker.address().city());

        //Entering state using java faker
        inputState.sendKeys(faker.address().state());

        //Entering zipcode using java faker
        String zipcode = faker.address().zipCode().replaceAll("-",""); //Zipcode u bazen"-" dash li getiriyor. bu invalid format o yuzden "replaceAll("-","")" ekledik.
        inputZip.sendKeys(zipcode);

        //inputZip.sendKeys(faker.address().zipCode().replaceAll("-","")); //2. yol
        ...
        WebElement successMessage = driver.findElement(By.xpath("//div[@class='buttons_process']/strong")); //strong is coming from parent(div)
...
/*benimkiler
 WebDriver driver;
    ...
    public void p1_smartbear_login_test(){
        ...
        for (WebElement link : allLinks){
            System.out.println(link.getText()); // " soutv " shorcut
        }
